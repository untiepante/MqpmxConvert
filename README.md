...
① プログラムを開きます (Launch this program.)  
② 変換したい.pmxファイルを選択します (In a file browser dialog, select source pmx file.)  
③ 変換してできる.mqo・.mqxファイルの名前を指定します。標準では元の名前の後ろに「.cnv.mqo」を付与した名前になっています (In a file browser dialog, decide the name of a destination file.)  
④ 変換が実施されコンソールウィンドウに「正常終了」と表示されます (After the conversion finishes, "正常終了" will be displayed.)  
